...
- deeper data checks (embedded COG overview statistics, Parquet 2.x native geospatial statistics)
...
`--live` additionally runs the live-hosting pass, probing the servers behind absolute `https` asset hrefs for the Data Storage MUSTs: a ranged `GET` (sent with an `Origin` header, since servers omit every `Access-Control-*` header without one) checks `206`/`Accept-Ranges` and the simple-response CORS headers, an `OPTIONS` preflight checks the allowed methods and request headers, and a `HEAD` per asset checks `Content-Length` (against the declared `file:size` when present). Range and CORS are server properties, so those probes run once per distinct host. Relative hrefs are not probeable from a local tree (a `--base-url` mapping is a planned follow-up); when nothing is probeable, or a host is unreachable, the pass degrades to `PTL-LIV-000` warnings.
...
| `PTL-LIV` | 000–005 | the hosting server's Data Storage MUSTs, probed per host over absolute `https` asset hrefs: ranged GET honored (`206`, `Accept-Ranges: bytes`), HEAD `Content-Length` present and matching `file:size`, CORS origin allowed, required headers exposed, preflight accepting `GET`/`HEAD` with `Range`; `000` warns when nothing is probeable or a host is unreachable |
